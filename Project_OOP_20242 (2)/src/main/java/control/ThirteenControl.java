...
public class ThirteenControl {
    ...
    private String gameType = "Thirteen";
    ListOfCards Deck = new ListOfCards();
    private GameRule rule;

    public ArrayList<Actor> getPlayersInGame() {
        return playersInGame;
    }

    public ListOfCards getDeck() {
        Deck.initializeDeck(gameType);
        return Deck;
    }

    public ThirteenControl(int numberOfPlayers, int numberOfBots, String gameType, GameRule rule) {
        Deck.initializeDeck(gameType);
        this.gameType = gameType;
        for (int i = 0; i < numberOfPlayers; i++) {
            Player p = new Player(gameType);
            p.setRule(gameType);
            playersInGame.add(p);
        }
//        for (int i = 0; i < numberOfBots; i++) {
//            Bot b = new Bot(gameType);
//            b.setRule(rule.toString());
//            playersInGame.add(b);
//        }
